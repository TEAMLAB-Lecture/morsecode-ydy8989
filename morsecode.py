...
def is_validated_morse_code(user_input):
    ...
    for morse_code in user_input.split():
        if morse_code in morse_code_dict.values():
            return True
        else:
            return False
    # ==================================
...
def get_cleaned_english_sentence(raw_english_sentence):
    ...
    return ''.join([c for c in raw_english_sentence.strip() if c not in stopwords])
    # ==================================
...
def decoding_sentence(morse_sentence):
    ...
    for word in morse_sentence.split('  '):
        result.append(''.join([decoding_character(code) for code in word.split()]))
    result = ' '.join(result)
...
def encoding_sentence(english_sentence):
    ...
    for word in get_cleaned_english_sentence(english_sentence).split():
        result.append(' '.join([encoding_character(c) for c in word]))
    result = '  '.join(result)
